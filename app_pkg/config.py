...
# ------------------------------------------- TP/SL бэктестов (BLOCK-37)
# ЕДИНАЯ пара уровней на весь проект: сканер (train и test окна) и
# /api/backtest/trades считают ОДИН и тот же прогон — с TP/SL, а не по
# сигнальным выходам. Поэтому число сделок в панели сканера и число блоков на
# графике совпадают 1:1, и у каждой сделки есть tp_price/sl_price
# (пунктирные IN/OUT/TP/SL). Уровни фиксируются на баре входа по ATR(14):
# TP = entry + BACKTEST_TP_ATR×ATR, SL = entry − BACKTEST_SL_ATR×ATR.
...
BACKTEST_TP_ATR = 2.0     # Take Profit = 2×ATR от entry
BACKTEST_SL_ATR = 1.0     # Stop Loss = 1×ATR от entry

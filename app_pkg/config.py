...
# ------------------------------------------- live-свечи в реальном времени
# Период фонового цикла, который пушит live_bars в SSE (событие candle_update).
# 1с — между двумя тиками в 1m-свечи проходит минута, поэтому секунда
# даёт «мгновенное» обновление графика без лишней нагрузки на SSE-клиентов.
SSE_CANDLE_PUSH_INTERVAL = 1.0

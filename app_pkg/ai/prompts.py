...
def charon_prompt_text() -> str:
    """[DEPRECATED] Системный промпт AI Backtest из config/charon_prompt.txt.

    LLM убрана из сигнального контура: текст файла больше не влияет на
    вердикт (его считает app_pkg.ai.apply_rules.apply_all_rules). Функция
    сохранена для обратной совместимости: levels_for_slice по-прежнему
    передаёт результат в _llm_request (детерминированный шов), а вкладка
    «🧠 Данные для ИИ» продолжает читать/писать файл как документацию
    правил (tooltips фронтенда).

    Файл создаётся с дефолтными правилами при первом обращении. Пустой
    файл трактуется как дефолт.
    """
